...
ADN_descriptors = function(vector, n, target = T, targetname){
  
  if (n == 1) {
    res = t(sapply(vector, prob_1))
    res = as.data.frame(res)
  } else if (n == 2) {
    res = t(sapply(vector, prob_2))
    res = as.data.frame(res)
  } else if (n == 3){
    res = t(sapply(vector, prob_3))
    res = as.data.frame(res)
  } else if (n == 4){
    res = t(sapply(vector, prob_4))
    res = as.data.frame(res)
  } else if (n == 5){
    res = t(sapply(vector, prob_5))
    res = as.data.frame(res)
  } else if (n == 6){
    res = t(sapply(vector, prob_6))
    res = as.data.frame(res)
  } else if (n == 7){
    res = t(sapply(vector, prob_7))
    res = as.data.frame(res)
  } else if (n == 8){
    res = t(sapply(vector, prob_8))
    res = as.data.frame(res)
  }
  
  if (target == T) {
    res = cbind(res, target = rep(targetname, nrow(res)))
  }
